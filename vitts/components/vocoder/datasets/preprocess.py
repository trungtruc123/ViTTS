...
def preprocess_wav_files(
    ...
    wave_files = find_wav_files(data_path)
    for path in tqdm(wave_files):
        ...
        if isinstance(mode, int):
            quant = ap.mulaw_encode(y, qc=mode) if mulaw else ap.quantize(y, bits=mode)
...
def find_feat_files(data_path):
    feat_path = glob.glob(os.path.join(data_path, "**", "*.npy"), recursive=True)
    return feat_path
...
def load_wav_feat_data(data_path, feat_path, eval_split_size):
    ...
    feat_paths = find_feat_files(data_path)

    wav_paths.sort(key=lambda x: Path(x).stem)
    feat_paths.sort(key=lambda x: Path(x).stem)

    assert len(wav_paths) == len(feat_paths), f"! {len(wav_paths)} != vs {len(feat_paths)} "
