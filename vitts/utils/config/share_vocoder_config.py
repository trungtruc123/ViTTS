...
from vitts.utils.config.config_base import BaseTrainingConfig, BaseAudioConfig
...
class BaseVocoderConfig(BaseTrainingConfig):
    ...
    Melspectrográm => Wave form
    Args:
        audio (BaseAudioConfig):
            Audio processor config instance. Defaults `BaseAudioConfig()`.
    ...
    eval_split_size: int = 10  # number of sample used for evaluation
    # dataset
    data_path: str = ""  # root_path. It find all wav file recursively from there
    feature_path: str = None  # if you use precomputed features
    seq_len: int = 1000  # signal length used in training
    pad_short: int = 0  # additional padding for short wav
...
class BaseGanVocoderConfig(BaseVocoderConfig):
    ...
    lr_gen: float = 0.0002  # init learning rate
    lr_disc: float = 0.0002  # init learning rate
